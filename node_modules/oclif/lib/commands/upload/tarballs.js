...
class UploadTarballs extends command_1.Command {
    async run() {
        ...
        for (const target of buildConfig.targets) {
            const tarball = dist(upload_util_1.templateShortKey('versioned', Object.assign({ ext: '.tar.gz', bin: config.bin, version, sha: gitSha }, target)));
        ...
        const uploadTarball = async (options) => {
            ...
            const releaseTarballs = async (ext) => {
                ...
                const cloudKey = `${upload_util_1.commitAWSDir(version, gitSha, s3Config)}/${localKey}`;
            ...
            const cloudKey = `${upload_util_1.commitAWSDir(version, gitSha, s3Config)}/${manifest}`;
        ...
        log_1.log(`done uploading tarballs & manifests for v${config.version}-${gitSha}`);
    }
}
