...
async function validateIndexJson(scenarioPath) {
    if (!acceptableLayouts) {
        acceptableLayouts = await layouts_1.getLayouts();
    }
    ...
    if (!acceptableLayouts.includes(index.environment.uilayout) && !acceptableLayouts.map(layout => layout.id).includes(index.environment.uilayout)) {
        return `Validation Error: Layout ${index.environment.uilayout} is not part of the acceptable layouts as specified here: https://www.katacoda.community/essentials/layouts.html`;
    }
...
async function validateExistingImage(imageid) {
    ...
    if (isValid == false) {
        return `Validation Error: Image ${imageid} is not part of the acceptable environment images as specified here: https://www.katacoda.community/essentials/environments.html`;
    }
    else {
        return true;
    }
